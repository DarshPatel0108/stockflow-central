...
export default function Transfers() {
  ...
  const filtered = transfers.filter((t) => {
    const matchSearch = t.ref.toLowerCase().includes(search.toLowerCase()) || t.from.toLowerCase().includes(search.toLowerCase()) || t.to.toLowerCase().includes(search.toLowerCase());
          ...
          <tbody className="divide-y divide-border">
            {filtered.map((t) => (
              <tr key={t.id} className="hover:bg-muted/30 transition-colors cursor-pointer">
                ...
                <td className="px-4 py-2.5"><StatusBadge status={t.status as DocStatus} /></td>
              </tr>
            ))}
            {filtered.length === 0 && (
